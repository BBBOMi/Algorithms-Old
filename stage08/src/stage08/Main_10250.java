 ...
 * 초기에 모든 방이 비어있다고 가정하에 이 정책에 따라 N번째로 도착한 손님에게 배정될 방 번호를 계산하는 프로그램이다.
 * 방 번호는 YXX 나 YYXX 형태인데 여기서 Y 나 YY 는 층 수를 나타내고 XX 는 엘리베이터에서부터 세었을 때의 번호를 나타낸다.
 * 첫 번째 손님은 101 호, 두 번째 손님은 201 호 등과 같이 배정한다. 
 * 예를 들면, H = 6이므로 10 번째 손님은 402 호에 배정해야 한다
 * 
 * <입력> 표준 입력에서 입력 데이터를 받는다.입력은 T 개의 테스트 데이터로 이루어져 있는데 T 는 입력의 맨 첫 줄에 주어진다.
...
public class Main_10250 {
	...
	public static int assignRooms(int h, int w, int n) {
	    int floor = (n%h != 0) ? n % h : h;
		int no = (floor == h) ? n / h + 1 : n / h;
